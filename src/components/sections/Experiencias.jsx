...
export default function Experiencias() {
    return (
        <Container className='px-[15px] md:px-[25px] lg:px-[50px] pt-[90px] overflow-hidden relative max-w-[1320px]'>
            <Titulo className="mb-0">Haz de tu viaje una experiencia inolvidable</Titulo>
            <Parrafo className="text-center mb-[40px]"><span className="font-semibold">Diseñemos juntos el mejor itinerario</span> a tu medida con tours personalizados para que conozcas los lugares que realmente deseas visitar, a tu ritmo y creando experiencias inolvidables con las personas que amas.</Parrafo>
...
const GaleriaUI = ({galeria = []}) => {
    ...
    return (
        <div className="flex flex-wrap justify-center">
            {galeria.map((g,k) => (
                <figure className="w-1/2 sm:w-1/3 md:w-1/3 xl:w-1/5" key={"galeria-"+IDGaleria+"-"+k}>
                    <img src={g} alt="Imagen galeria" className="object-cover" />
                </figure>
            ))}
        </div>
    )
